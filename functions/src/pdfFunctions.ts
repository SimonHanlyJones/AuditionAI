...
export const getScriptAndConvert = functions.https.onRequest(
    ...
    if (request.headers.authorization === "duckduck") {
      try {
        ...
        pdfParser.on("pdfParser_dataError", (err: Error) => {
          ...
          response
            .status(500)
            .send(`Failed to extract text from PDF : ${err.message}`);
        });
        ...
        response
          .status(500)
          .send(`Failed to extract text from PDF: ${error.message}`);
      }
    } else {
