...
// Define the secret
const googleGeminiApiKey = functions.params.defineSecret(
  "GOOGLE_GEMINI_API_KEY",
);
const googleVoiceApiKey = functions.params.defineSecret("GOOGLE_VOICE_API_KEY");

exports.helloWorld = functions.https.onRequest(
  { secrets: [googleGeminiApiKey, googleVoiceApiKey] },
...
export const callGemini = functions.https.onRequest(
  { timeoutSeconds: 500, secrets: [googleGeminiApiKey] },
  async (request: any, response: any) => {
    if (
      request.method === "POST" &&
      request.headers.authorization === "duckduck"
    ) {
      try {
