...
async function getUniqueFilename() {
    ...
    const dateString = date.toISOString().replace(/:/g, '').replace(/\..+/, '').replace(/T/g, '_').replace(/-/g, '');
...
export async function synthesizeVoiceFromTextApiCall(text: string, voiceName: string, gender: string): Promise<string> {

    if (!text || !voiceName || !gender || !validateVoice( voiceName, gender)) {
        console.error(`Voice API call input error: text: ${text}, voiceName: ${voiceName}, gender: ${gender}`);
        throw new Error(`Voice API call input error: text: ${text}, voiceName: ${voiceName}, gender: ${gender}`);
    }
    ...
    try {
        ...
        if (response.ok) {
            console.log(`Speech synthesized for text: ${text}, voiceName: ${voiceName}, gender: ${gender}`);
