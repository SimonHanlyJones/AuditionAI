...
export const HelloWorldButtonFromAPI = () => {
  return (
    <Button
      title="Test API is connected with this weird trick"
...
export const getTitleAndCharactersButton = (script: string) => {
  return (
    <Button
      title="Get Title and Characters"
      onPress={() => getTitleAndCharacters(script)}
    />
  );
};
...
export const getAnalysisButton = (script: string, character: string) => {
  return (
    <Button
      ...
      onPress={() => getCharacterAnalysis(script, character)}
    />
  );
};

const generateAndPlayVoice = async (text: string) => {
  const path = await synthesizeVoiceFromTextApiCall(
    text,
    "en-GB-Neural2-A",
    "FEMALE",
  );
...
export const VoiceTestButton = () => {
  return (
    <Button
      ...
      onPress={() => generateAndPlayVoice("hi, I'm a bird! Look at me fly!")}
    />
  );
};
...
export const ScriptAnalysisComponentDemoComponent = () => {
  ...
  const performAnalysis = async () => {
    ...
    try {
      const analysisData = await getCharacterAnalysis(
        script,
        selectedCharacter,
      );
  ...
  return (
    <View>
      <Button
        title="TEST BUTTON: Get PDF or Txt Script"
        onPress={handleScriptSelection}
      />
      {script && (
        <Button
          title="Get Title and Characters"
          onPress={fetchTitleAndCharacters}
        />
      )}
      {characters.length > 0 && (
        <View>
